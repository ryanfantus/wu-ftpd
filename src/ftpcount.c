/****************************************************************************  
  ...
  $Id: ftpcount.c,v 1.11 2011/11/28 01:54:15 wmaton Exp $
...
static int acl_fgetlimit(char *aclbuf, char *class)
{
	...
	if (strncasecmp(aclbuf, "limit", 5) == 0) {
	    ...
	    *--crptr = '\0';
	    strlcpy(linebuf, aclbuf, sizeof(linebuf));
	    *crptr = '\n';
...
#  else /* !(defined(AIX)) */ 
#    if defined (LINUX)
		    snprintf(line, sizeof(line), "/bin/ps fwwwp %d", procid);
#    else /* !(defined (LINUX) */ 
...
#endif /* !(defined(SVR4)) */ 
		    ZeFile = popen(line, "r");
		    (void) fgets(line, sizeof(line), ZeFile);
		    line[0] = '\0';
		    (void) fgets(line, sizeof(line), ZeFile);
		    if (line[0] != '\0') {
			size_t i;
	...
	*(aclbuf + (size_t) finfo.st_size) = '\0';
	...
	if (strncasecmp(myaclbuf, "class", 5) == 0) {
	    ...
	    *crptr = '\n';
		...
		limit = acl_fgetlimit(myaclbuf, class);
#if defined(VIRTUAL)
		if (address != NULL)
		    printf("%s ", address);
#endif /* defined(VIRTUAL) */ 
