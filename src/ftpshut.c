/****************************************************************************  
  ...
  $Id: ftpshut.c,v 1.10 2011/11/28 01:54:16 wmaton Exp $
...
#  endif /* !(defined(INET6)) */ 
	...
	(void) fread(aclbuf, finfo.st_size, 1, accessfile);
	*(aclbuf + finfo.st_size) = '\0';
	...
	if (strncasecmp(myaclbuf, "shutdown", 8) == 0) {
	    ...
	    *--crptr = '\0';
	    strlcpy(linebuf, myaclbuf, sizeof(linebuf));
	    *crptr = '\n';
	    (void) strtok(linebuf, " \t");	/* returns "shutdown" */
	    sp = strtok(NULL, " \t");	/* returns shutdown path */
	    strlcpy(shutmsg, sp, sizeof(shutmsg)); /* save for future use */
	}
	...
	snprintf(anonpath, sizeof(anonpath), "%s%s", pwent->pw_dir, shutmsg);
	if (newfile(anonpath))
	...
	if (strncasecmp(linebuf, "virtual", 7) == 0) {
		...
		*sp = '\0';	/* truncate blanks, comments etc. */
		...
		snprintf(altmsgpath, sizeof(altmsgpath), "%s%s", cp, shutmsg);
	...
	while (read_servers_line(svrfp, hostaddress, sizeof(hostaddress), configdir, sizeof(configdir)) == 1) {
		...
		*sp = '\0';
	    ...
	    snprintf(accesspath, sizeof(accesspath), "%s/ftpaccess", configdir);
		...
		snprintf(altmsgpath, sizeof(altmsgpath), "%s%s", root, cp);
