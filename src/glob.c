/****************************************************************************    
  ...
  $Id: glob.c,v 1.10 2011/10/20 22:58:10 wmaton Exp $ 
...
#    include <unistd.h>
static long NCARGS=0;
static int GAVSIZ=0;

static void get_NCARGS()
{
    if (NCARGS == 0)
        NCARGS = sysconf(_SC_ARG_MAX)/6;
    if (NCARGS < 0)
 fatal("unknown _SC_ARG_MAX");
    GAVSIZ=NCARGS/6;
}
...
char **ftpglob(register char *v)
{
    ...
    get_NCARGS();
    agargv=(char **) malloc((sizeof (char *)) * GAVSIZ);
    if (agargv == (char **) 0)
	fatal("Out of memory");
#endif /* !(defined(GAVSIZ)) */ 
