/****************************************************************************
  ...
  $Id: routevector.c,v 1.9 2011/10/20 22:58:11 wmaton Exp $  
...
{
    unsigned int addr32[4], rem32[4];
    int bitstozero, i, start = 0, len = sizeof(unsigned int);
    char *ptr;
	...
	bitstozero = 32 - sig;
	memcpy(addr32, addr, sizeof(unsigned int));
	memcpy(rem32, ptr, sizeof(unsigned int));
#if defined(INET6)
    }
    else {
