/****************************************************************************    
  ...
  $Id: paths.c,v 1.9 2011/10/20 22:58:11 wmaton Exp $  
...
#  endif /* !(defined(UNIXWARE) || defined(AIX)) */ 
    ...
    strlcpy(_path_ftpaccess, _PATH_FTPACCESS, sizeof(_path_ftpaccess));
    strlcpy(_path_ftpusers, _PATH_FTPUSERS, sizeof(_path_ftpusers));
    strlcpy(_path_private, _PATH_PRIVATE, sizeof(_path_private));
    strlcpy(_path_cvt, _PATH_CVT, sizeof(_path_cvt));
    strlcpy(logfile, _PATH_XFERLOG, MAXPATHLEN);
    strlcpy(_path_ftphosts, _PATH_FTPHOSTS, sizeof(_path_ftphosts));
	...
	if (getsockname(0, (struct sockaddr *) &virtual_addr, &virtual_len) == 0) {
	    while (read_servers_line(svrfp, hostaddress, sizeof(hostaddress), configdir, sizeof(configdir)) == 1) {
		if (!strcmp(hostaddress, inet_stop(&virtual_addr))) {
		    if (debug)
			syslog(LOG_DEBUG, "VirtualFTP Connect to: %s", hostaddress);
		    strlcpy(virtual_address, hostaddress, MAXHOSTNAMELEN);
		    if (hostname != NULL) {
			/* reset hostname to this virtual name */
			wu_gethostbyaddr(&virtual_addr, hostname, MAXHOSTNAMELEN);
			strlcpy(virtual_hostname, hostname, MAXHOSTNAMELEN);
		    }
			...
			*sp = '\0';
			...
			snprintf(filepath, sizeof(filepath), "%s/ftpaccess", configdir);
			if (access(filepath, R_OK) == 0) {
			    strlcpy(_path_ftpaccess, filepath, sizeof(_path_ftpaccess));
			    virtual_mode = 1;
			    virtual_ftpaccess = 1;
			}

			snprintf(filepath, sizeof(filepath), "%s/ftpusers", configdir);
			if (access(filepath, R_OK) == 0)
			    strlcpy(_path_ftpusers, filepath, sizeof(_path_ftpusers));

			snprintf(filepath, sizeof(filepath), "%s/ftpgroups", configdir);
			if (access(filepath, R_OK) == 0)
			    strlcpy(_path_private, filepath, sizeof(_path_private));

			snprintf(filepath, sizeof(filepath), "%s/ftphosts", configdir);
			if (access(filepath, R_OK) == 0)
			    strlcpy(_path_ftphosts, filepath, sizeof(_path_ftphosts));

			snprintf(filepath, sizeof(filepath), "%s/ftpconversions", configdir);
			if (access(filepath, R_OK) == 0)
			    strlcpy(_path_cvt, filepath, sizeof(_path_cvt));
		    }
		    (void) fclose(svrfp);
		    return;
		}
